...
import RetrieveFileBox from "@/components/RetrieveMode/RetrieveFileBox.tsx";


function RetrieveTypeFile() {
    ...
    const onGenrate = async () => {
        const response = await axios.post(`${baseUrl}/retrieve_file`, {
            key: otp,
        })
        setViewUrl(response.data.textOrUrl)
    }
    return (
        <>
            <div>
                ...
                <div className="flex justify-center items-center ">
                    <InputOTP id="digits-only" maxLength={6} value={otp} onChange={(value) => setOtp(value)}
                ...
                <div>
                    ...
                    {showFile && <RetrieveFileBox Url={viewUrl}/>}
                </div>
            </div>
        </>
    )
...
export default RetrieveTypeFile;